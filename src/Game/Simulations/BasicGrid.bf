...
public class BasicGrid : Simulation
{
	public override void Setup(int luigiIndex, ref List<Face> faces)
	{
		...
		if (faces.Count >= (SCREEN_WIDTH * SCREEN_HEIGHT) / (FACE_WIDTH * FACE_HEIGHT))
		{
			gridColumns = 8; // Hack, I can't be bothered to figure out the math for this right now.
		}

		let gridWidth = (uint32)gridColumns * FACE_WIDTH;
		let gridHeight = (uint32)gridRows * FACE_HEIGHT;

		// Center grid to screen
		let gridStartX = ((SCREEN_WIDTH - gridWidth) / 2) + (FACE_WIDTH / 2);
		let gridStartY = ((SCREEN_HEIGHT - gridHeight) / 2) + (FACE_HEIGHT / 2);
